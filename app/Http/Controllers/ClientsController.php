...
class ClientsController extends Controller
{
  ...
  public function anyData(Request $request)
  {
    ...
    if ($request->user_id) {
      $clients->where('user_id', '=', $request->user_id);
    }
    if ($request->source_id) {
      $clients->where('source_id', '=', $request->source_id);
    }
    if ($request->agency) {
      $clients->where('agency_id', '=', $request->agency);
    }
    if ($request->status) {
      $clients->where('status', '=', $request->status);
    }
    if ($request->priority) {
      $clients->where('priority', '=', $request->priority);
    }
    if ($request->client_number) {
      $clients->Where('client_number', 'LIKE', '%' . $request->client_number . '%')
        ->orWhere('client_number_2', 'LIKE', '%' . $request->client_number . '%');
    }
    if ($request->country) {
      $clients->Where('country', 'LIKE', '%' . $request->country . '%');
    }
    if ($request->filterDateBase !== 'none')
    {
      ...
      if($request->daysActif){
          $clients->where('updated_at', '<=', \Carbon\Carbon::today()->subDays($request->daysActif));
      }
    ...
    return Datatables::of($clients)
      ...
      ->editColumn('client_number', function ($clients) {
        return $clients->client_number . '<br>' . $clients->client_number_2;
      })
      ->editColumn('client_email', function ($clients) {
        return '<a href="mailto:' . $clients->client_email . '">' . $clients->client_email . '</a><br><a href="mailto:' . $clients->client_email_2 . '">' . $clients->client_email_2 . '</a>';
      })
